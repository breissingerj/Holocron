@~/.config/opencode/instructions/steering-rules.md

@$HOLOCRON_MEMORY_DIR/memory/MEMORY.md

# Holocron (Claude Code)
...
The steering rules above (steering-rules.md) are already loaded. Internalize and apply them in every response.
...
**MANDATORY FIRST ACTION:** Read `~/.claude/instructions/algorithm.md`, then follow that file's instructions exactly. Do NOT improvise your own algorithm format — switch all processing and responses to the actual Algorithm in that file until it completes.
...
- **Session naming (CRITICAL)** — At the start of EVERY response where the task is clear, immediately run `/rename <label>` with a short slug describing the active work (e.g., `fix-auth-bug`, `lahzo-pixel-tracking`, `opencode-research`). Update the name if the task pivots. Never leave a session unnamed or with a stale label. Do not ask — just rename.
