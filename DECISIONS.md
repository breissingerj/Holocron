...
### Spec 001 Phase 2 (Foundational): T004, T005, T007 completed; T008 blocked

- **Decision** — Executed the three Foundational tasks not requiring a private-repo file that turned out to be missing: (1) **T004** — `instructions/algorithm.md` was already a superset of `claude/instructions/algorithm.md`'s reflect learnings (12 vs. 8, all 8 present), so the merge work was: split the single OpenCode-flavored capability table into labeled "Claude Code tools" / "pi tools" subsections, replace the two hardcoded `~/.config/opencode/scripts/voice.sh` voice-announcement paths with `$HOLOCRON_DIR/scripts/voice.sh`, strip remaining OpenCode text (the `opencode.json`/LSP-diagnostics row, the "requires OpenCode hosted plan" websearch line), then delete `claude/instructions/`. (2) **T005** — `instructions/AGENTS.md` was already the near-final canonical draft; fixed its 3 remaining hardcoded `~/.config/opencode/...` paths to `$HOLOCRON_DIR/...` and added the FR-005 documented-fallback note. `pi/AGENTS.md` contributed nothing to the merge — it is 100% pi-specific (Graphiti + TillDone), correctly deferred to T010's `APPEND_SYSTEM.md`. (3) **T007** — `git mv claude/agents agents` (top-level) + `git rm -r opencode/agents/`.
- **Options considered (pi tool table, T004)** — (1) Relabel the existing OpenCode-flavored table as "pi tools" unchanged (rejected — OpenCode's tool names (`webfetch`, `websearch`, `diagnostics`, lowercase `agent`) are not pi's actual API; this would make the Algorithm instruct pi sessions to call tools that don't exist). (2) Guess pi's tool surface from the Constitution/spec's general description (rejected — violates the "empirical verification over documentation assumptions" constraint). (3) Verify empirically against the installed `@earendil-works/pi-coding-agent@0.83.0` package (chosen) — confirmed built-ins are `read`, `write`, `edit`, `bash`, `glob`, `grep`, `ls`, `skill`; no native webfetch/websearch tool exists; a `subagent` tool is referenced only by this repo's `subagent-progress.ts` extension UI code, with no corresponding registration found in the installed core package — documented as "unconfirmed, verify with a live tool call before relying on it" rather than asserted as available.
- **Rationale** — Matches Constitution IV (no claim about tool availability without verification) and III (a capability table that lists non-existent tools would be a phantom-capability generator every time an Algorithm session ran under pi). T004/T005/T007 had no open design questions once the empirical pi-tool question was resolved — the bulk of the "merge" was already done by prior (uncommitted) edits to the two canonical files; this session's job was closing the remaining harness-specific-path gaps and doing the file moves.

### T008 blocked — private agent source files no longer exist in the memory repo

- **Observation** — T008 (move `$HOLOCRON_MEMORY_DIR/agents/opencode/{LahzoChatTester,ProductManager}.md` → `$HOLOCRON_MEMORY_DIR/agents/`) cannot execute: neither `agents/opencode/` nor `agents/claude/` exists in the memory repo's working tree, and no trace of either filename exists anywhere else in the repo. `git log` in the memory repo shows both directories (4 files total) were deleted in commit `acaca00` ("vault backup: 2026-08-07 14:50:44") — an automated Obsidian-git sync commit, not a deliberate migration commit — three weeks *before* spec 001's `research.md` (2026-08-28, R5) asserted these files still lived at `agents/opencode/`. Research R5's premise was already stale the day it was written.
- **Decision** — Jack confirmed (2026-08-31) both private agents are intentionally retired, not to be restored. T008 is marked retired in `tasks.md`; its downstream US4 clauses are dropped — T019 no longer links a `$HOLOCRON_MEMORY_DIR/agents/` root, T021's acceptance check drops the private-agent-linked criterion (AS5), and `data-model.md`'s Agent entity + Pointer Inventory row 10 are updated to reflect zero private agents.
- **Options considered** — Restore from `acaca00^:agents/claude/*.md` (the pre-deletion Claude-format copies) vs. treat as intentionally retired and drop scope. Retirement chosen per Jack's direction.
- **Rationale** — An unexplained 3-week-old deletion via an automated backup commit is exactly the kind of surprising discovery that changes a task's premise; recording it here (rather than only in `tasks.md`) makes it discoverable independent of task-list churn.

### Spec 001 — User Stories 1–7 + Polish implemented; live machine migrated

- **Decision** — Completed all remaining tasks (T009–T035; T036 left for Jack's sign-off) in one session. `install.sh` was rewritten from scratch (granular per-harness copy-paste, OpenCode-only) into a single convergent installer for exactly two harnesses: a `converge_entry()` primitive classifies and repairs every live pointer (MISSING/STALE/DANGLING/CHURNED/PRECEDENCE), `--check` runs the same classification read-only with the drift table + exit-code contract from `contracts/install-cli.md`, and `merge_link_skills()` was extended to auto-detect and repair legacy CamelCase-named live entries left over from T006's rename — including CamelCase→hyphenated cases (`ContentAnalysis`→`content-analysis`) that a pure case-fold comparison would miss, since APFS's case-insensitivity alone doesn't help there. The live machine was fully migrated: 65 pointers converged on the first apply run, `--check` now reports `CLEAN`, and a churn drill (swap `~/.claude/settings.json` from symlink to real file, `--check`→CHURNED, apply→RESET, `--check`→clean) and a fresh-clone dry run (sandboxed empty `$HOME`, 74 changes then 0) both passed. `~/.config/opencode/` had its remaining 7 Holocron-created pointers removed with a full notice (T029); the directory is left empty, untouched going forward.
- **Options considered** — For the pi capability table in `algorithm.md` (T004, prior session) and the `pi-subagents`/`APPEND_SYSTEM.md` questions revisited this session: assert documented behavior vs. verify against the installed package. Chose verification each time — confirmed `~/.pi/agent/APPEND_SYSTEM.md` auto-load is real (found in `resource-loader.js`, not just documented), and conversely that the documented `pi-subagents` npm package (`pi/EXTENSIONS.md`, "Installed 2026-05-01") is **not actually installed** on this machine (`pi list` reports nothing, no package directory exists) — a real doc/reality drift, left as-is (out of scope) but noted in `holocron-system.md` so it isn't silently treated as available. For SC-006 (pi zero skill-name warnings) and SC-002 (live marker prompt), several non-interactive `pi --print`/`--verbose` flag combinations were tried to get a clean startup signal without a full interactive session; none produced usable output, so both are recorded as source/file-verified rather than live-session-verified rather than fabricating a session transcript.
- **Rationale** — Matches Constitution IV (verify, don't assert) and III (no phantom capabilities) throughout; the CamelCase-rename edge case and the `pi-subagents` drift are exactly the kind of gap that empirical checking (vs. trusting the spec's own prior research or the repo's own docs) catches. Two known follow-up gaps were surfaced rather than silently patched or silently ignored: `scripts/memory-feed.sh` has no data producer since its OpenCode plugin was deleted, and `claude/validate-claude-cli.sh`/`ValidateClaudeCLI.md`/`CLAUDE_CLI_COMPATIBILITY.md` were marked superseded by `install.sh --check` rather than line-by-line rewritten (their dual-harness checks are now obsolete by design, not broken by accident).

### PR follow-up: memory-feed producer wired, validation scripts rewritten, orphan-agent gap found and fixed

- **Decision** — Per Jack's request to implement the two follow-up gaps flagged in the PR: (1) **memory-feed producer** — the initial "no Claude Code replacement exists" claim was wrong. `claude/scripts/hooks/memory-feed.sh` (a Claude-native PostToolUse Write|Edit hook, functionally identical to the deleted OpenCode plugin) already existed in the public `claude/settings.json` template; it was simply never added to this machine's live `$HOLOCRON_MEMORY_DIR/settings.json`. Added it to that file's `Write`/`Edit` matchers (alongside `PRDSync.hook.ts`) and verified end-to-end (`echo '{"tool_input":{"file_path":".../LEARNING/SIGNALS/test.jsonl"}}' | bash ~/.claude/scripts/hooks/memory-feed.sh` correctly appends a classified line). Also found and fixed a related dangling reference: the live settings.json's `HOLOCRON_CONFIG_DIR` env var still pointed at the now-emptied `~/.config/opencode/` — repointed to `~/.claude`. (2) **Validation scripts** — fully rewrote `claude/validate-claude-cli.sh` and `claude/ValidateClaudeCLI.md` for the single-harness layout instead of leaving them as superseded stubs: CLAUDE.md checks now verify generated-concatenation content (marker line, `Execution Modes` heading, `claude-tail.md` content) instead of dead `@`-import greps; skills/agents checks now expect real merge-linked directories instead of top-level symlinks; the hook-scripts section now reads every command actually wired in `settings.json` and checks each referenced file, rather than a hardcoded list at the retired opencode path; the MCP check now verifies `obsidian` registration (what `install.sh` actually manages) instead of a `linear`/`enabledMcpjsonServers` check that was already stale for unrelated reasons. `CLAUDE_CLI_COMPATIBILITY.md` kept as-is (historical reference, its banner already accurate).
- **Found via the rewrite, not anticipated** — running the new `validate-claude-cli.sh` surfaced a real bug the T032 machine migration missed: 2 dangling symlinks at `~/.claude/agents/{LahzoChatTester,ProductManager}.md`, pointing at the `$HOLOCRON_MEMORY_DIR/agents/claude/` paths from before T008 was retired. `merge_link_agents()` only converges files present in its *source* directory — it never scanned the *destination* for orphans with no matching source, so these pre-existing dangling links were invisible to `install.sh --check` even though it reported `CLEAN`. Removed the 2 links directly, then fixed `merge_link_agents()` in `install.sh` to detect and remove dangling orphans (a symlink in the destination with no matching source file, whose target doesn't exist) on every run going forward — verified with a synthetic orphan (`--check` reports it, apply removes it).
- **Options considered** — For the orphan-agent fix: leave it as a one-time manual cleanup vs. fix the installer so it can't silently recur. Chose to fix the installer — this is exactly the kind of rot US5 ("the machine converges, and drift is visible") was supposed to close, and leaving the detection gap would mean the next agent deletion (intentional or not) creates the same invisible dangling link again.
- **Rationale** — This is a case where writing the *validation* tooling surfaced a defect in the *implementation* tooling from the same session — a genuine reason to keep both: `install.sh --check` classifies machine-wide pointer drift, but its per-source convergence loop structurally can't see orphans; a content-level validator that enumerates the actual destination catches what the pointer-level one doesn't.
