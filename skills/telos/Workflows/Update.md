...
TELOS is {PRINCIPAL.NAME}'s life framework stored in `$HOLOCRON_MEMORY_DIR/Holocron/USER/TELOS/`. It contains:
...
!`FILE="$1"; CONTENT="$2"; DESCRIPTION="$3"; bun $HOLOCRON_MEMORY_DIR/commands/update-telos.ts "$FILE" "$CONTENT" "$DESCRIPTION"`
...
!`head -50 $HOLOCRON_MEMORY_DIR/Holocron/USER/TELOS/updates.md`

## View Specific TELOS File
!`FILE="$1"; cat $HOLOCRON_MEMORY_DIR/Holocron/USER/TELOS/"$FILE"`
...
Example:
```bash
bun $HOLOCRON_MEMORY_DIR/commands/update-telos.ts "BOOKS.md" "- *Project Hail Mary* by Andy Weir" "Added favorite book: Project Hail Mary"
```
...
The script is at: `$HOLOCRON_MEMORY_DIR/commands/update-telos.ts`

All backups are stored in: `$HOLOCRON_MEMORY_DIR/Holocron/USER/TELOS/Backups/`

All changes are logged in: `$HOLOCRON_MEMORY_DIR/Holocron/USER/TELOS/updates.md`
