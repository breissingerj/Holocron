...
- Keep responses concise but informative`
    ...
    const inferenceResult = await new Promise<{ success: boolean; output?: string; error?: string }>((resolve) => {
      ...
      const proc = spawn('bun', ['run', `${process.env.HOLOCRON_MEMORY_DIR || homeDir + "/Projects/personalProjects/holocron-context"}/PAI/Tools/Inference.ts`, '--level', 'fast', systemPrompt, message], {
