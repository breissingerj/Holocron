...
```bash
$HOLOCRON_MEMORY_DIR/WORK/{current_work}/
```

**To get the current work directory:**
1. Read `$HOLOCRON_MEMORY_DIR/STATE/current-work.json`
2. Extract the `work_dir` value
3. Use `$HOLOCRON_MEMORY_DIR/WORK/{work_dir}/` for temporary artifacts
...
```
$HOLOCRON_MEMORY_DIR/WORK/20260111-172408_extract-alpha-analysis/
├── raw-transcript.txt
...
WORK_DIR=$(jq -r '.work_dir' $HOLOCRON_MEMORY_DIR/STATE/current-work.json)

# 2. Work in current work item directory
cd $HOLOCRON_MEMORY_DIR/WORK/${WORK_DIR}/
...
2. **Get current work directory** - Read `$HOLOCRON_MEMORY_DIR/STATE/current-work.json` for `work_dir`
