 ...
 *
 * @see $HOLOCRON_DIR/skills/art/SKILL.md
 */
...
async function main() {
  try {
    ...
    if (!botToken) {
      throw new CLIError(
        'Missing DISCORD_BOT_TOKEN environment variable. Add it to ${HOLOCRON_DIR}/.env'
      );
    }

    if (!channelId) {
      throw new CLIError(
        'Missing MIDJOURNEY_CHANNEL_ID environment variable. Add it to ${HOLOCRON_DIR}/.env'
      );
    }
