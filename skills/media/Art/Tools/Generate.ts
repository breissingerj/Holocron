 ...
 *
 * @see $HOLOCRON_DIR/skills/art/README.md
 */
...
/**
 * Load environment variables from ${HOLOCRON_DIR}/.env
...
const HOLOCRON_DIR = process.env.HOLOCRON_DIR || join(process.env.HOME!, "Projects", "personalProjects", "holocron-context");
...
MORE INFO:
  Documentation: ${HOLOCRON_DIR}/skills/media/Art/README.md
  Source: ${HOLOCRON_DIR}/skills/media/Art/Tools/Generate.ts
`);
  process.exit(0);
}
...
async function main(): Promise<void> {
  try {
    // Load API keys from ${HOLOCRON_DIR}/.env
    await loadEnv();
