...
### Skill Directory
```bash
ls $HOLOCRON_DIR/skills/ | grep -i [skillname]
```
...
```bash
ls -la $HOLOCRON_DIR/skills/[SkillName]/
```
...
```bash
bun $HOLOCRON_DIR/skills/[SkillName]/Tools/[ToolName].ts --help
```
...
```bash
grep -l "Intent-to-Flag" $HOLOCRON_DIR/skills/[SkillName]/Workflows/*.md
```
...
**Reference:** `$HOLOCRON_DIR/CliFirstArchitecture.md`
