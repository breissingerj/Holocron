...
- $HOLOCRON_DIR/USER/TELOS/TELOS.md
- $HOLOCRON_DIR/USER/TELOS/GOALS.md
- $HOLOCRON_DIR/USER/TELOS/PROJECTS.md
- $HOLOCRON_DIR/USER/TELOS/CHALLENGES.md
- $HOLOCRON_DIR/USER/TELOS/STATUS.md
...
- Read $HOLOCRON_MEMORY_DIR/STATE/current-work.json
...
- List skills in $HOLOCRON_DIR/skills/
...
1. Load channel config:
   bun $HOLOCRON_DIR/Tools/LoadSkillConfig.ts ../youtube-channels.json
...
4. For NEW videos, extract transcripts:
   bun $HOLOCRON_DIR/Tools/GetTranscript.ts '<video-url>'
...
1. Look in $HOLOCRON_DIR/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/
...
1. Load the github_trending config from user-sources.json:
   Read $HOLOCRON_DIR/USER/SKILLCUSTOMIZATIONS/PAIUpgrade/user-sources.json
