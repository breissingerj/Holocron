...
bun run $HOLOCRON_DIR/skills/Utilities/Evals/EvalServer/cli.ts results \
  --use-case <name> \
  --latest
```
...
bun run $HOLOCRON_DIR/skills/Utilities/Evals/EvalServer/cli.ts report \
  --run-id <run-id> \
  --output $HOLOCRON_DIR/skills/Utilities/Evals/Results/<use-case>/<run-id>/report.md
```
...
```bash
bun run $HOLOCRON_DIR/skills/Utilities/Evals/EvalServer/cli.ts compare \
  --run-a <run-id-1> \
  --run-b <run-id-2>
```
...
```bash
bun run $HOLOCRON_DIR/skills/Utilities/Evals/EvalServer/cli.ts trend \
  --use-case <name> \
  --days 30
```
