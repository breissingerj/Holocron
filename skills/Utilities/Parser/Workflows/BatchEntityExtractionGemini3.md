...
const entityIndex = JSON.parse(
  fs.readFileSync('$HOLOCRON_DIR/skills/parser/entity-index.json', 'utf-8')
);
...
```typescript
import { scrapeAsMarkdown } from '$HOLOCRON_DIR/skills/mcp/Providers/brightdata/actors';
...
// Main processing
const entityIndexPath = '$HOLOCRON_DIR/skills/parser/entity-index.json';
...
const schema = JSON.parse(
  fs.readFileSync('$HOLOCRON_DIR/skills/parser/schema/content-schema.json', 'utf-8')
);
...
processedArticles.forEach((article: any, index: number) => {
  ...
  if (!valid) {
    ...
    const filename = `$HOLOCRON_DIR/skills/parser/output/${timestamp}_batch-${index + 1}.json`;
...
```bash
# Check entity counts
cat $HOLOCRON_DIR/skills/parser/entity-index.json | jq '{
...
URLS=("$@")
SKILL_DIR="${HOLOCRON_MEMORY_DIR}/skills/Utilities/Parser"
...
const entityIndexPath = `${process.env.HOLOCRON_MEMORY_DIR}/skills/Utilities/Parser/entity-index.json`;
...
processedArticles.forEach((article: any, index: number) => {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
  const filename = `${process.env.HOLOCRON_MEMORY_DIR}/skills/Utilities/Parser/output/${timestamp}_batch-${index + 1}.json`;
