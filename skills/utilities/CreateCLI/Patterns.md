...
function loadConfig(): Config {
  ...
  try {
    ...
    if (!apiKey) {
      console.error('Error: API_KEY not found in ${HOLOCRON_MEMORY_DIR}/.env');
    ...
    console.error('Error: Cannot read ${HOLOCRON_MEMORY_DIR}/.env');
    console.error('Create file: touch ${HOLOCRON_MEMORY_DIR}/.env');
    process.exit(1);
  }
}
```
...
**Key principles:**
- Load from ${HOLOCRON_MEMORY_DIR}/.env (PAI standard)
...
CONFIGURATION:
  API Key: ${HOLOCRON_MEMORY_DIR}/.env (API_KEY=your_key)
...
For full documentation: $HOLOCRON_DIR/tools/bin/${CLI_NAME}/README.md
Version: ${VERSION}
`);
}
```
...
- [ ] Configuration loading (from ${HOLOCRON_MEMORY_DIR}/.env)
