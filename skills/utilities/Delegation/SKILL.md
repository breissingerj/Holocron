...
| "**create an agent team**", "**agent team**", "**swarm**" | **Agent Teams** (Claude Code) | `TeamCreate` → `TaskCreate` → `SendMessage` | Persistent team with shared task list, message coordination, multi-turn collaboration. Not available on pi. |
...
bun run $HOLOCRON_DIR/skills/agents/Tools/ComposeAgent.ts --traits "security,skeptical,thorough" --task "Review auth" --output json
