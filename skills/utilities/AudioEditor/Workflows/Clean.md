...
```bash
bun $HOLOCRON_DIR/skills/utilities/AudioEditor/Tools/Pipeline.ts \
  "<audio-file-path>" \
...
```bash
# Transcription only
bun $HOLOCRON_DIR/skills/utilities/AudioEditor/Tools/Transcribe.ts <file>

# Analysis only (requires transcript)
bun $HOLOCRON_DIR/skills/utilities/AudioEditor/Tools/Analyze.ts <transcript.json>

# Edit only (requires audio + edits)
bun $HOLOCRON_DIR/skills/utilities/AudioEditor/Tools/Edit.ts <file> <edits.json>

# Polish only (requires CLEANVOICE_API_KEY)
bun $HOLOCRON_DIR/skills/utilities/AudioEditor/Tools/Polish.ts <file>
```
