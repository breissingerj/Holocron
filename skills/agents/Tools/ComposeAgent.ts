 ...
 *
 * Configuration files:
 *   Base:  $HOLOCRON_DIR/skills/agents/Data/Traits.yaml
 *   User:  $HOLOCRON_MEMORY_DIR/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
 *
 * Usage:
...
const CUSTOM_AGENTS_DIR = `${HOLOCRON_MEMORY_DIR}/agents/custom`;
...
/**
 * Save a composed agent to $HOLOCRON_MEMORY_DIR/custom-agents/{slug}.md
 *
 * Produces a CLAUDE CODE COMPATIBLE agent file that can be:
 * 1. Copied to $HOLOCRON_MEMORY_DIR/agents/ and used as a built-in agent
 ...
 *
 * Matches the structural format of built-in agents in $HOLOCRON_MEMORY_DIR/agents/*.md:
...
\`\`\`bash
bun run $HOLOCRON_DIR/skills/agents/Tools/ComposeAgent.ts --load "${slug}"
\`\`\`

Or reconstruct from traits:

\`\`\`bash
bun run $HOLOCRON_DIR/skills/agents/Tools/ComposeAgent.ts --traits "${agent.traits.join(",")}"
\`\`\`
...
OPTIONS:
  ...
  -s, --save           Save composed agent to $HOLOCRON_MEMORY_DIR/custom-agents/
...
CONFIGURATION:
  Base traits:    $HOLOCRON_DIR/skills/agents/Data/Traits.yaml
  User traits:    $HOLOCRON_MEMORY_DIR/PAI/USER/SKILLCUSTOMIZATIONS/Agents/Traits.yaml
  Custom agents:  $HOLOCRON_MEMORY_DIR/custom-agents/
