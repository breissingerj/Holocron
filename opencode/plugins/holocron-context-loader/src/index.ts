...
export async function getMostRecentPRD(workDir: string): Promise<string | null> {
  try {
...
export async function buildContextBlock(_$: unknown): Promise<string | null> {
...
export const HolocronContextLoader: Plugin = async ({ $, client }) => {
  return {
     ...
     */
    "tui.prompt.append": async (_input: unknown, output: any) => {
