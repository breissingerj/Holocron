...
**Location:** `$HOLOCRON_MEMORY_DIR/projects/-Users-{username}--claude/`
...
ls $HOLOCRON_MEMORY_DIR/WORK/ | tail -5
```

### Check ratings
```bash
tail $HOLOCRON_MEMORY_DIR/LEARNING/SIGNALS/ratings.jsonl
```
...
ls -lt $HOLOCRON_MEMORY_DIR/projects/-Users-{username}--claude/*.jsonl | head -5

# View last session events
tail $HOLOCRON_MEMORY_DIR/projects/-Users-{username}--claude/$(ls -t $HOLOCRON_MEMORY_DIR/projects/-Users-{username}--claude/*.jsonl | head -1) | jq .
```

### Check learnings
```bash
ls $HOLOCRON_MEMORY_DIR/LEARNING/SYSTEM/
ls $HOLOCRON_MEMORY_DIR/LEARNING/ALGORITHM/
ls $HOLOCRON_MEMORY_DIR/LEARNING/SYNTHESIS/
```

### Check failures
```bash
# List recent failure captures
ls -lt $HOLOCRON_MEMORY_DIR/LEARNING/FAILURES/$(date +%Y-%m)/ 2>/dev/null | head -10
...
```bash
ls $HOLOCRON_MEMORY_DIR/STATE/progress/
```
...
- Moved BACKUPS to `$HOLOCRON_MEMORY_DIR/BACKUPS/` (outside MEMORY)
- Renamed RAW-OUTPUTS to RAW
- All directories now ALL CAPS

**2026-01-05:** v1.0 - Unified Memory System migration
- Previous: `$HOLOCRON_MEMORY_DIR/history/`, `$HOLOCRON_MEMORY_DIR/context/`, `$HOLOCRON_MEMORY_DIR/progress/`
- Current: `$HOLOCRON_MEMORY_DIR/`
