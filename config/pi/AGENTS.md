...
Pi.dev loads this file by concatenation (no `@file` imports). Referenced instruction files (`algorithm.md`, `steering-rules.md`) are symlinked into `~/.pi/agent/instructions/` by Holocron's `install.sh`; read them with the `read` tool when a mode requires them.

---

## Behavioral Rules

Before doing any work, read and internalize `~/.pi/agent/instructions/steering-rules.md`.
...
**Voice:** `bash ~/.pi/agent/scripts/voice.sh "Executing using native mode"`
...
**MANDATORY FIRST ACTION:** Read `~/.pi/agent/instructions/algorithm.md`, then follow that file's instructions exactly. Do NOT improvise your own algorithm format — switch all processing and responses to the actual Algorithm in that file until it completes.
...
- **Session naming** — Pi uses `/name <name>` to set a session display name. At the start of EVERY response where the task is clear, run `/name <slug>` with a short slug describing the active work (e.g., `fix-auth-bug`, `lahzo-pixel-tracking`). Update the name if the task pivots. Never leave a session unnamed or with a stale label.
