...
- [ ] T006 [P] Rename the 13 CamelCase public skills in `skills/` to lowercase-hyphen slugs (edit `name` in each frontmatter so `name` == directory); update every in-repo reference to the old CamelCase paths (grep `skills/` references across `instructions/`, `skills/` cross-references, `agents/`, `commands/` — a grep for each old name must return nothing)
