...
**Full voice registry:** `~/.config/opencode/skills/Agents/SKILL.md` (see Named Agents) and `$HOLOCRON_MEMORY_DIR/settings.json` (daidentity.voiceId)
...
Located in `$HOLOCRON_MEMORY_DIR/settings.json`:
...
The notification service is in `$HOLOCRON_MEMORY_DIR/hooks/lib/notifications.ts`:
