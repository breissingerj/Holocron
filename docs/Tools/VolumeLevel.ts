/**
 * VolumeLevel.ts — Shared volume level reader for PAI voice/notification system
 *
 ...
 *
 * Backward compat: if volume.level is absent but voice.muted exists → returns 0.
 */
...
const STATE_DIR = join(process.env.HOLOCRON_MEMORY_DIR || join(homedir(), 'Projects', 'personalProjects', 'holocron-context'), 'STATE');
...
export function getVolumeLevel(): VolumeLevel {
  // Read new-style volume.level file
