...
21+ event hooks across the session lifecycle: SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, Stop, SessionEnd. Defined in `settings.json`, implemented in `$HOLOCRON_MEMORY_DIR/hooks/`.
...
- **Add a hook:** Create handler in `$HOLOCRON_MEMORY_DIR/hooks/handlers/`, register in `settings.json`
- **Add startup files:** Append to `settings.json → loadAtStartup.files`
- **Add user context:** Create files in `Holocron/USER/`
