...
**Location:** `$HOLOCRON_MEMORY_DIR/hooks/`
**Configuration:** `$HOLOCRON_MEMORY_DIR/settings.json`
...
- **History Capture** - Automatic work/learning documentation to `$HOLOCRON_MEMORY_DIR/`
  ...
  "SessionStart": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/KittyEnvPersist.hook.ts"
        },
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/LoadContext.hook.ts"
        }
      ]
    }
  ]
  ...
  "SessionEnd": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/WorkCompletionLearning.hook.ts"
        },
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/SessionCleanup.hook.ts"
        },
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/RelationshipMemory.hook.ts"
        },
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/UpdateCounts.hook.ts"
        },
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/IntegrityCheck.hook.ts"
        }
      ]
    }
  ]
  ...
  "UserPromptSubmit": [
    {
      "hooks": [
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/RatingCapture.hook.ts"
        },
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/UpdateTabTitle.hook.ts"
        },
        {
          "type": "command",
          "command": "${HOLOCRON_MEMORY_DIR}/hooks/SessionAutoName.hook.ts"
        }
      ]
    }
  ]
...
```json
{
  "Stop": [
    {
      "hooks": [
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/LastResponseCache.hook.ts" },
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/ResponseTabReset.hook.ts" },
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/VoiceCompletion.hook.ts" },
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/DocIntegrity.hook.ts" },
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/AlgorithmTab.hook.ts" }
      ]
    }
  ]
}
  ...
  "PreToolUse": [
    {
      "matcher": "Bash",
      "hooks": [
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/SecurityValidator.hook.ts" }
      ]
    },
    {
      "matcher": "Edit",
    ...
    },
    {
      ...
      "hooks": [
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/SetQuestionTab.hook.ts" }
      ]
    },
    {
      "matcher": "Task",
      "hooks": [
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/AgentExecutionGuard.hook.ts" }
      ]
    },
    {
      "matcher": "Skill",
      "hooks": [
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/SkillGuard.hook.ts" }
      ]
    }
  ]
}
  ...
  "PostToolUse": [
    {
      "matcher": "AskUserQuestion",
      "hooks": [
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/QuestionAnswered.hook.ts" }
      ]
    },
    {
      "matcher": "Write",
      "hooks": [
        { "type": "command", "command": "${HOLOCRON_MEMORY_DIR}/hooks/PRDSync.hook.ts" }
      ]
    },
    {
      "matcher": "Edit",
...
### Location
**File:** `$HOLOCRON_MEMORY_DIR/settings.json`
**Section:** `"hooks": { ... }`

### Environment Variables
Hooks have access to all environment variables from `$HOLOCRON_MEMORY_DIR/settings.json` `"env"` section:

```json
{
  "env": {
    "HOLOCRON_MEMORY_DIR": "$HOME/.claude",
...
**Key Variables:**
- `HOLOCRON_MEMORY_DIR` - Holocron context repo root (hooks, skills, PAI docs all live here)
- Hook scripts reference `${HOLOCRON_MEMORY_DIR}` in command paths
    ...
    "HookEventName": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "${HOLOCRON_MEMORY_DIR}/hooks/my-hook.ts --arg value"
          }
        ]
      }
    ]
...
**Session Mapping:** `$HOLOCRON_MEMORY_DIR/STATE/agent-sessions.json`
```json
{
...
**Location:** `$HOLOCRON_MEMORY_DIR/hooks/my-custom-hook.ts`
...
```bash
chmod +x $HOLOCRON_MEMORY_DIR/hooks/my-custom-hook.ts
```
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "${HOLOCRON_MEMORY_DIR}/hooks/my-custom-hook.ts"
          }
        ]
      }
    ]
...
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun $HOLOCRON_MEMORY_DIR/hooks/my-custom-hook.ts
```
...
- Use `${HOLOCRON_MEMORY_DIR}` in settings.json for portability
- Access in code via `process.env.HOLOCRON_MEMORY_DIR`
...
**Check:**
1. Is hook script executable? `chmod +x $HOLOCRON_MEMORY_DIR/hooks/my-hook.ts`
2. Is path correct in settings.json? Use `${HOLOCRON_MEMORY_DIR}/hooks/...`
3. Is settings.json valid JSON? `jq . $HOLOCRON_MEMORY_DIR/settings.json`
4. Did you restart Claude Code after editing settings.json?

**Debug:**
```bash
# Test hook directly
echo '{"session_id":"test","transcript_path":"/tmp/test.jsonl","hook_event_name":"Stop"}' | bun $HOLOCRON_MEMORY_DIR/hooks/my-hook.ts

# Check hook logs (stderr output)
tail -f $HOLOCRON_MEMORY_DIR/hooks/debug.log  # If you add logging
```
...
**Check:**
1. Does `$HOLOCRON_MEMORY_DIR/` directory exist?
2. Does current-work file exist? Check `$HOLOCRON_MEMORY_DIR/STATE/current-work.json`
3. Is hook actually running? Check `$HOLOCRON_MEMORY_DIR/RAW/` for events
4. File permissions? `ls -la $HOLOCRON_MEMORY_DIR/WORK/`

**Debug:**
```bash
# Check current work
cat $HOLOCRON_MEMORY_DIR/STATE/current-work.json

# Check recent work directories
ls -lt $HOLOCRON_MEMORY_DIR/WORK/ | head -10
ls -lt $HOLOCRON_MEMORY_DIR/LEARNING/$(date +%Y-%m)/ | head -10

# Check raw events
tail $HOLOCRON_MEMORY_DIR/RAW/$(date +%Y-%m)/$(date +%Y-%m-%d)_all-events.jsonl
```
...
**Check:**
1. Is `$HOLOCRON_MEMORY_DIR/STATE/agent-sessions.json` writable?
...
cat $HOLOCRON_MEMORY_DIR/STATE/agent-sessions.json | jq .

# Check subagent-stop debug log
tail -f $HOLOCRON_MEMORY_DIR/hooks/subagent-stop-debug.log
```
...
grep '"type":"user"' $HOLOCRON_MEMORY_DIR/projects/-Users-username--claude/*.jsonl | head -1 | jq '.type'
...
bun $HOLOCRON_MEMORY_DIR/hooks/LoadContext.hook.ts
...
- File not found → Check `HOLOCRON_MEMORY_DIR` environment variable
- Permission denied → `chmod +x $HOLOCRON_MEMORY_DIR/hooks/LoadContext.hook.ts`
...
```json
{
  "Stop": [
    {
      "hooks": [
        { "command": "${HOLOCRON_MEMORY_DIR}/hooks/Stop hooks.hook.ts" }  // Single orchestrator
      ]
    }
  ]
}
...
KEY FILES:
$HOLOCRON_MEMORY_DIR/settings.json              Hook configuration
$HOLOCRON_MEMORY_DIR/hooks/                     Hook scripts (22 files)
$HOLOCRON_MEMORY_DIR/hooks/handlers/            Handler modules (6 files)
$HOLOCRON_MEMORY_DIR/hooks/lib/                 Shared libraries (13 files)
$HOLOCRON_MEMORY_DIR/hooks/lib/learning-utils.ts Learning categorization
$HOLOCRON_MEMORY_DIR/hooks/lib/time.ts          PST timestamp utilities
$HOLOCRON_MEMORY_DIR/hooks/lib/event-types.ts   Typed event definitions (22 interfaces)
$HOLOCRON_MEMORY_DIR/hooks/lib/event-emitter.ts appendEvent() → events.jsonl
$HOLOCRON_MEMORY_DIR/WORK/               Work tracking
$HOLOCRON_MEMORY_DIR/LEARNING/           Learning captures
$HOLOCRON_MEMORY_DIR/STATE/              Runtime state
...
| File | Purpose |
|------|---------|
| `${HOLOCRON_MEMORY_DIR}/hooks/lib/event-types.ts` | TypeScript discriminated union of all PAI event types (22 interfaces covering algorithm, work, session, rating, learning, voice, PRD, doc, build, system, tab, hook error, and custom events) |
| `${HOLOCRON_MEMORY_DIR}/hooks/lib/event-emitter.ts` | `appendEvent()` utility that writes typed events to `${HOLOCRON_MEMORY_DIR}/MEMORY/STATE/events.jsonl` |
...
tail -f $HOLOCRON_MEMORY_DIR/STATE/events.jsonl | jq

# Filter by type
tail -f $HOLOCRON_MEMORY_DIR/STATE/events.jsonl | jq 'select(.type | startswith("algorithm."))'
